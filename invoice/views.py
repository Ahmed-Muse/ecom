...
def anonymous_required(function=None, redirect_url=None):
   ...
   if not redirect_url:
       redirect_url = 'invoice_dashboard'
...
def invoice_dashboard(request):
    ...
    return render(request,'invoice_dashboard.html',context)

@anonymous_required#you must be logged out for you to access the login page
def login(request):
    context = {}
...
def invoice_dashboard(request):
    ...
    clients = ClientsModel.objects.all().count()
    invoices = InvoicesModel.objects.all().count()#this gives the total no. of invoices
    paidInvoices = InvoicesModel.objects.filter(status='Paid').count()
...
def createBuildInvoice(request, slug):
    ...
    if request.method == 'GET':
        ...
        inv_form = InvoiceForm(instance=invoice)#invoice is the instance of the form..if you dont specify the instance, it will auto create instance every time
...
def viewPDFInvoice(request, slug):
    ...
    #fetch all the products - related to this invoice
    products = ProductsModel.objects.filter(invoice=invoice)

    #Get Client Settings
    p_settings = SettingsModel.objects.get(clientName='Allifmaal Ltd')
    ...
    if len(products) > 0:
        for x in products:
            y = float(x.quantity) * float(x.unitPrice)
            invoiceTotal += y
    ...
    return render(request, 'invoice-template.html', context)
...
def viewDocumentInvoice(request, slug):
    ...
    #Get Client Settings
    p_settings = SettingsModel.objects.get(clientName='ahmeddove114@gmail.com')
...
def emailDocumentInvoice(request, slug):
    ...
    #Get Client Settings
    p_settings = SettingsModel.objects.get(clientName='Skolo Online Learning')
...
def deleteInvoice(request, slug):
    try:
        InvoicesModel.objects.get(slug=slug).delete()
    except:
        messages.error(request, 'Something went wrong')
        return redirect('invoices')
...
def companySettings(request):
    company = SettingsModel.objects.get(clientName='Skolo Online Learning')
